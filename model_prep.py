...
class TrendModel:
    ...
    def train_model(self, target_period=10, feat_types=['Volume', 'Seasonal', 'Trend', 'Volatility'], method='xgb',
        ...
        if method == 'xgb':
            self.model = self.ml.xgb_model(params, evaluate=True, eval_log=self.model_info['Dir'] + save_file, plot_pred=plot)

        if method == 'gbr':
            self.model = self.ml.tree_model(params, gbr=True, evaluate=True, eval_log=self.model_info['Dir'] + save_file,
                                plot_pred=plot)
        if method == 'xgbclf':
